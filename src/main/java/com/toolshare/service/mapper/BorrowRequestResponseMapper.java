...
/**
 * BorrowRequest -> BorrowRequestResponse 的统一转换组件。
 * 单条与批量共用同一套字段映射，工具名、申请人名、是否已评价统一按 ID 批量加载，避免 N+1。
 */
@Component
public class BorrowRequestResponseMapper {
    ...
    public BorrowRequestResponse toResponse(BorrowRequest borrowRequest) {
        return toResponseList(Collections.singletonList(borrowRequest)).get(0);
    }
    ...
    public PageResponse<BorrowRequestResponse> toPageResponse(Page<BorrowRequest> page) {
